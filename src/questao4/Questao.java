...
public class Questao {
	public static void main(String[] args) {
		...
		// Questão 4.a -> pegando uma string e transformando em um objeto do tipo
		// LocalDate.
		LocalDate data = LocalDate.parse(dataDigitada, formatador);

		// Questão 4.b -> Pegando data de hj e calculando a diferença para a data
		// digitada em dias.
	...
	public static int calcularDiasuteis(LocalDate data1, LocalDate data2) {
		...
		for (int contador = 0; contador <= diferencaEmDias; contador++) {
			if (data1.compareTo(data2) < 0) {
				if ((data1.plusDays(contador).getDayOfWeek() == DayOfWeek.SATURDAY
						|| data1.plusDays(contador).getDayOfWeek() == DayOfWeek.SUNDAY)) {
					contagemDias--;
				}
			} else {
				if ((data2.plusDays(contador).getDayOfWeek() == DayOfWeek.SATURDAY
						|| data2.plusDays(contador).getDayOfWeek() == DayOfWeek.SUNDAY)) {
					contagemDias--;
				}
			}
		}
